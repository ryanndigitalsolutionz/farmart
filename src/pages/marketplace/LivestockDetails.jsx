...
export default function LivestockDetails() {
  ...
  return (
    <div style={{ maxWidth: 1100, margin: "0 auto", padding: "28px 20px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "minmax(280px, 1fr) minmax(300px, 1fr)", gap: 24, alignItems: "start" }}>
        <div style={{ background: "#EAF3E6", borderRadius: 12, height: 340, display: "flex", alignItems: "center", justifyContent: "center", color: "var(--text-muted, #66766A)" }}>
          {listing.images?.[0] ? <img src={listing.images[0]} alt={listing.title} style={{ width: "100%", height: "100%", objectFit: "cover", borderRadius: 12 }} /> : "No image available"}
        </div>
        <div>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "start", gap: 10 }}>
            <h1 style={{ fontFamily: "'IBM Plex Serif', serif", fontSize: 24, color: "var(--text-dark, #1E2A1F)", margin: "0 0 6px" }}>{listing.title}</h1>
            <button onClick={toggleWishlist} style={{ background: "none", border: "none", fontSize: 22, cursor: "pointer", color: wishlisted ? "#DC2626" : "var(--text-muted, #66766A)" }}>{wishlisted ? "♥" : "♡"}</button>
          </div>
          <div style={{ color: "var(--text-muted, #66766A)", fontSize: 14, marginBottom: 10 }}>{listing.farmerName} • {listing.location}</div>
          ...
          <div style={{ marginBottom: 14 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 6 }}>
              <button onClick={() => setQty((q) => Math.max(1, q - 1))} style={{ width: 32, height: 32, borderRadius: 8, border: "1px solid var(--border, #DCE6D8)", background: "#fff", cursor: "pointer" }}>-</button>
              <span style={{ fontWeight: 700, minWidth: 24, textAlign: "center" }}>{qty}</span>
              <button onClick={() => setQty((q) => Math.min(listing.quantity, q + 1))} style={{ width: 32, height: 32, borderRadius: 8, border: "1px solid var(--border, #DCE6D8)", background: "#fff", cursor: "pointer" }}>+</button>
            </div>
          </div>
          <button onClick={handleAddToCart} disabled={listing.quantity <= 0} style={{ width: "100%", padding: "12px 16px", background: "var(--green-700, #2F6D3F)", color: "#fff", border: "none", borderRadius: 10, fontSize: 15, fontWeight: 700, cursor: "pointer", opacity: listing.quantity <= 0 ? 0.5 : 1 }}>
