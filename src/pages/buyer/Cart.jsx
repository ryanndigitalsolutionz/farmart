      ...
      {items.length === 0 ? (
        ...
        <div style={{ display: "grid", gridTemplateColumns: "minmax(0, 2fr) minmax(240px, 1fr)", gap: 20, alignItems: "start" }}>
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {items.map((item) => (
              <div key={item.listingId} style={{ background: "var(--white, #fff)", border: "1px solid var(--border, #DCE6D8)", borderRadius: 12, padding: 14, boxShadow: "0 6px 20px rgba(29,78,42,0.05)", display: "flex", gap: 14, alignItems: "center" }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <button onClick={() => handleQty(item.listingId, item.quantity - 1)} style={{ width: 28, height: 28, borderRadius: 6, border: "1px solid var(--border, #DCE6D8)", background: "#fff", cursor: "pointer" }}>-</button>
                ...
                <button onClick={() => handleRemove(item.listingId)} style={{ background: "none", border: "none", color: "#DC2626", cursor: "pointer", fontSize: 18 }}>×</button>
              </div>
            ))}
          </div>
