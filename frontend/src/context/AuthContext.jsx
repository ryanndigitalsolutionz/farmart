...
export function AuthProvider({ children }) {
  ...
  useEffect(() => {
    const legacyKey = "farmartUser";
    if (storage.get(legacyKey, null) && !storage.get("currentUser", null)) {
      try {
        const legacyUser = storage.get(legacyKey, null);
        if (legacyUser) {
          setUser(legacyUser);
        }
      } catch {
  ...
  const registerWithUser = useCallback((userData) => {
    setUser(userData);
  }, []);
